...
public class QAgent extends Agent {
	 ...
	 * You can make it as deep as you want to (although it will take more time to
	 * compute)
	 * 
	 ...
	 * received on the last turn.
	 * This is where you will check for things like Did this footman take or give
	 * damage? Did this footman die
	 ...
	 * taken damage. You can use
	 * the history view to get a list of damages dealt in the previous turn. Use
	 ...
	 * oldFeatureVectors field
	 * so that when that action ends (and we observe a transition to a new state),
	 * we can update the Q value Q(s,a)
	 *
	 ...
	 * scenario is the current
	 * state view and the history of this episode. The action is the attacker and
	 * the enemy pair for the
	 * SEPIA attack action.
	 *
	 * This returns the Q-value according to your feature approximation. This is
	 * where you will pass
	 * your features through your network (and extract the predicted q-value using
	 * the .item() method)
	 * 
	...
	private Matrix getTDGroundTruth(StateView state, HistoryView history, int unitId) throws Exception {
		if (!this.getOldInfoPerUnit().containsKey(unitId)) {
			throw new Exception(
					"unitId=" + unitId + " does not have an old feature vector...cannot calculate TD ground truth for it");
		}
	 ...
	 * check if an event has occurred. If it has then you will need to update your
	 * weights and select a new action.
	 *
	 * If you are using the footmen vectors you will also need to remove killed
	 * units. To do so use the historyView
	 * to get a DeathLog. Each DeathLog tells you which player's unit died and the
	 * unit ID of the dead unit. To get
	 * the deaths from the last turn do something similar to the following snippet.
	 * Please be aware that on the first
	 ...
	 * you never want a footman just
	 * sitting around doing nothing (the enemy certainly isn't going to stop
	 * attacking). So at the minimum you will
	 * have an event whenever one your footmen's targets is killed or an action
	 * fails. Actions may fail if the target
	 * is surrounded or the unit cannot find a path to the unit. To get the action
	 * results from the previous turn
	 * you can do something similar to the following. Please be aware that on the
	 * first turn you should not call this
	 *
		...
		for (Integer unitId : this.getMyUnitIds()) {
			...
			if (this.numTestEpisodesPlayedInBatch != -1) {
				this.totalRewards.set(this.totalRewards.size() - 1,
						this.totalRewards.get(this.totalRewards.size() - 1)
			...
			if (state.getTurnNumber() == 0 || !prevUnitActions.containsKey(unitId) ||
				...
				if (state.getTurnNumber() > 0) {
					...
					try {
						...
						System.err
								.println("QAgent.middleStep [ERROR]: problem updating gradients for transition on unitId=" + unitId);
						e.printStackTrace();
						System.exit(-1);
					}
				}
